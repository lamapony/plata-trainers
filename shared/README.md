...
`exercise-schema.json` and `plata-exercise-spec.js` define the rendering-independent exercise contract used by the technical track. Agents generate JSON specs with phenomena, stimulus, correct/distractor answers, weakTags, level, type, and metric requirements. `PlataExerciseSpec.validate(...)` checks the contract, `getStimulus(...)` and `getOptions(...)` expose render-ready data, `checkAnswer(...)` grades attempts, and `toKernelAttempt(...)` bridges into `PlataKernel.recordAttempt(...)`. Run `npm run check:exercise-schema` to verify the schema file, runtime validator, fixtures, and grading path.
