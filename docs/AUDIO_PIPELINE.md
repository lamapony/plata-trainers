...
4. New bytes are written to a temporary file, decoded for QC, then atomically renamed. Existing valid clips are reused by content hash. Orphans are reported but never deleted automatically.
...
- stable utterance ID, display `text`, exact `spokenText`, speaker, voice, source field, and utterance kind;
...
- decoded QC evidence: RMS/peak level, leading/trailing silence, and hard-cutoff risk;
...
- RMS outside −36 to −8 dBFS, clipping, more than 0.8 seconds of leading silence, more than 1 second of trailing silence, or a suspiciously loud final 20 ms;
...
MP3 frames and WAV containers are parsed in Node so CI can verify real structure without `ffmpeg`. Production generation additionally requires `ffmpeg` to decode the waveform for loudness, silence, and cutoff analysis.
...
- Listen/Pause/Replay uses a native button with a text label, keyboard focus, `aria-label`, and `aria-pressed` state.
...
- The service worker omits audio from install precache, stores complete audio lazily after the first request, and rotates its cache version when any asset bytes change.

## Recovery and continuation

If generation stops, rerun the same command. Valid hash-matching files are reused and invalid temporary files are cleaned. Inspect reported orphans manually; do not use a broad deletion command. A text or casting correction should be made in `data.js`, followed by dry-run, generation, validation, listening review, and release validation in that order.
