...
```text
sourceNotes -> learningGoal -> targetPhrases -> interaction -> feedback -> mastery signal
```
...
```text
source-backed claim -> scene goal -> trained phrase -> diagnostic interaction -> recorded mastery signal
```
