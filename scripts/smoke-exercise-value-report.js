...
function runBaseSmoke() {
  ...
  assert(report.totals.transferChains >= 1, "exercise value report should count transfer chains");
  assert(report.totals.exerciseChains >= 3, "exercise value report should count flagship and transfer chains together");
...
function runCliSmoke() {
  ...
  try {
    ...
    assert(JSON.parse(json.stdout).totals.exerciseChains >= 3, "CLI JSON should include exercise chain totals");
